...
def get_current_state():
    try:
        views_res = client.get(f"{API_URL}/servers/localhost/views")
        views_res.raise_for_status()
        views = set(views_res.json().get("views", []))
        views.add("default")

        response = client.get(f"{API_URL}/servers/localhost/networks")
        response.raise_for_status()
        nets_res = response.json()
...
def delete_view(view_name: str):
    ...
    try:
        ...
        for zone in res.json().get("zones", []):
            client.delete(f"{API_URL}/servers/localhost/views/{view_name}/{zone}").raise_for_status()
            client.delete(f"{API_URL}/servers/localhost/zones/{zone}").raise_for_status()
    except Exception as e:
...
def sync():
    ...
    desired_net_map = {}
    sorted_views = sorted(
        views_config.keys(), key=lambda k: views_config[k].get("priority", 0) if views_config[k] else 0
    )
    ...
    for view_name in sorted_views:
        if view_name != "default" and view_name not in current_views:
            ...
            client.post(
                f"{API_URL}/servers/localhost/views/{view_name}", json={"name": f"..{view_name}"}
            ).raise_for_status()
    ...
    for net, view in desired_net_map.items():
        if current_net_map.get(net) != view:
            ...
            client.put(f"{API_URL}/servers/localhost/networks/{net}", json={"view": view}).raise_for_status()

    # 4. Managed Deletions
    cleanup_stats = {}  # {old_view: count}
    if managed_only:
        for net, view in current_net_map.items():
            if net not in desired_net_map:
                cleanup_stats[view] = cleanup_stats.get(view, 0) + 1
                client.put(f"{API_URL}/servers/localhost/networks/{net}", json={"view": ""}).raise_for_status()
