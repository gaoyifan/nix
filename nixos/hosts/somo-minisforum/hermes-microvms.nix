{
  ...
  restartableVirtiofsd = pkgs.writeShellScriptBin "virtiofsd" ''
    ${lib.getExe pkgs.virtiofsd} "$@"
    # virtiofsd exits successfully when Cloud Hypervisor disconnects during a
    # guest reboot. Make supervisord restart it for the VMM's reconnect attempt.
    exit 1
  '';
  secretDirectory = vmName: "/run/${vmName}-secrets";
in {
  ...
  systemd.tmpfiles.settings."10-microvm" = lib.mapAttrs' (vmName: _:
    lib.nameValuePair "/var/lib/microvms/${vmName}/hermes" {
      d = {
        user = lib.mkForce "1000";
        group = lib.mkForce "1000";
  ...
  systemd.services = lib.mkMerge (
    lib.mapAttrsToList (vmName: vm: {
      "${vmName}-secrets" = {
        ...
        before = ["microvm-virtiofsd@${vmName}.service"];
        path = [
          pkgs.coreutils
          pkgs.openssl
        ];
        ...
        script = ''
          ...
          secrets_dir=${secretDirectory vmName}
    ...
    lib.mapAttrs (vmName: vm: {
      config = {
        ...
        managedSkills = pkgs.runCommand "hermes-managed-skills" {} ''
          ...
          cp -rL ${./hermes/skills/local-whisper-transcription} $out/local-whisper-transcription
        '';
        ...
        terminalImageShare = pkgs.runCommandLocal "hermes-terminal-image" {} ''
          mkdir -p $out
          cp --reflink=always ${terminalImage} $out/image.tar.zst
        '';
      in {
        ...
        microvm = {
          ...
          virtiofsd.package = restartableVirtiofsd;
          writableStoreOverlay = "/nix/.rw-store";
          preStart = ''
            test -r ${terminalImageShare}/image.tar.zst
          '';
          interfaces = [
            {
              type = "tap";
          ...
          shares = [
            {
              tag = "hermes";
              source = "/var/lib/microvms/${vmName}/hermes";
            ...
            {
              tag = "hermes-secrets";
              source = secretDirectory vmName;
              mountPoint = "/etc/hermes";
              proto = "virtiofs";
              readOnly = true;
            }
            {
              tag = "hermes-terminal-image";
              # microvm.nix also uses source as a tmpfiles attribute name.
              source = builtins.unsafeDiscardStringContext "${terminalImageShare}";
              mountPoint = "/run/hermes-terminal-image";
              proto = "virtiofs";
              readOnly = true;
              cache = "always";
            }
        ...
        fileSystems."/etc/hermes".neededForBoot = true;
        fileSystems."/run/hermes-terminal-image".neededForBoot = true;
        ...
        systemd.services.hermes-terminal-image = {
          ...
          unitConfig.RequiresMountsFor = "/var/lib/containers /var/lib/hermes /run/hermes-terminal-image";
          environment = {
          ...
          script = ''
            ...
            if ! podman image exists "$image_ref"; then
              podman load --input /run/hermes-terminal-image/image.tar.zst
            fi
