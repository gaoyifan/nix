{
  ...
  nodes.router = {
    ...
    exerciseTopology = pkgs.writeShellScriptBin "exercise-home-router-topology" ''
      ...
      ip -d link show dev br-core.22 | grep -F 'altname alpha'
      ip -d link show dev br-core.22 | grep -F 'altname beta'
      ...
      ip -4 route get 203.0.113.10 mark 198 | grep -F 'via 198.51.100.1 dev br-core.931 table native src 198.51.100.2'
      ip -4 route get 203.0.113.10 mark 201 | grep -F 'via 192.0.2.1 dev br-core.22 table alpha src 192.0.2.2'
      ip -4 route get 203.0.113.10 mark 202 | grep -F 'via 192.0.2.1 dev br-core.22 table beta src 192.0.2.3'

      timeout 5 ip netns exec upstream tcpdump -qnli upstream0.22 -c 1 'icmp and dst host 203.0.113.10' > /tmp/wlt-alpha-capture &
      capture_pid=$!
      sleep 0.2
      ip netns exec guest ping -c 1 -W 2 203.0.113.10
      wait "$capture_pid"
      grep -F '10.64.2.2 > 203.0.113.10' /tmp/wlt-alpha-capture
    '';
  in {
    imports = [
